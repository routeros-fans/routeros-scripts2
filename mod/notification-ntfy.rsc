    ...
    :if ([ :typeof $Message ] = "array" ) do={
      :do {
        /tool/fetch check-certificate=yes-without-crl output=none http-method=post \
          ($Message->"url") user=($Message->"user") password=($Message->"pass") \
          http-header-field=($Message->"headers") http-data=($Message->"text") as-value;
        :set ($NtfyQueue->$Id);
      } on-error={
...
:set ($NotificationFunctions->"ntfy") do={
  ...
  :do {
    ...
    /tool/fetch check-certificate=yes-without-crl output=none http-method=post \
      $Url user=$User password=$Pass http-header-field=$Headers http-data=$Text as-value;
  } on-error={
